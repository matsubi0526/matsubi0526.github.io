...
# $Lastupdate: 2014-10-28 17:43:54$
...
<html lang="{{ site.lang }}">
  <head>
    ...
    <link rel="stylesheet" href="{{ site.base_path }}/assets/bootstrap/css/bootstrap.min.css" type="text/css" media="all">
    <link rel="stylesheet" href="{{ site.base_path }}/assets/bootstrap/css/bootstrap-theme.min.css" type="text/css" media="all">
...
module RD
  ...
  class RD2HtmlDennouClubVisitor < RD::RD2HTMLExtVisitor
    def apply_to_Index(element, content)
      # str = content.join.gsub!(/(&lt;|&gt;|&amp;)/) {
      #    METACHAR.index($&)
      # }
      str = content.join.gsub(/&lt;/,"<").gsub(/&gt;/,">")
      %Q[#{str}]
    end
    ...
    def apply_to_Headline(element, title)
      ...
      title = title.join("")
      %Q[<h#{element.level}><a id="#{anchor}">#{title}</a></h#{element.level}>]
